...
**💎** 1. `AGREGAT(9;5;…)` ignore lignes masquées **et** erreurs. · 2. Le **mode plan** replie un rapport de 400 lignes en 12. · 3. `SOUS.TOTAL(109;…)` vs `SOUS.TOTAL(9;…)` : la différence entre masqué et filtré.
...
Elle conclut à une **baisse de 12 % à Douala**. En réalité, l'agence a changé de plan comptable en mars : deux familles de produits ont été reclassées. **`V4 — Vérité métier` l'attrape.** Aucun des trois autres V ne l'aurait vue.
